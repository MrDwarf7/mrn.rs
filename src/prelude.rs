    ...
    std::env::current_exe()
        ...
        .map_err(|e| {
            anyhow::Error::msg(format!(
                "Release mode: Failed to get current executable/binary: {e}"
            ))
        })
}
...
fn current_path() -> crate::prelude::Result<std::path::PathBuf> {
    std::env::current_dir()
        .map_err(|e| e.to_string())
        .map_err(|e| {
            anyhow::Error::msg(format!("Debug mode: Failed to get current directory: {e}"))
        })
